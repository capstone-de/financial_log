...
interface ApiInter {
    ...
    //지출 함꼐한 사람
    @GET("wallet_app/saveExpense?user=3")
    ...
    @GET("/diary_app/diaryList?user=3")
    fun getDiarylist(): Call<ArrayList<ResponseDiary>>
    @GET("/diary_app/myDiaryList?user=3")
    ...
    @GET("/statistics_app/yearly?user=3")
    fun getStatisticsYearly(@Query("year") year: String): Call<ResponseStatYear> //연별
    @GET("/wordcloud_app/myDiary?user=3")// 나의 현재 트렌드
