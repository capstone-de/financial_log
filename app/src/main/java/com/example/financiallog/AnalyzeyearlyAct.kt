    ...
    private fun showMoreMenu() {
        ...
        moreBottomView.setOnItemSelectedListener { item ->
            when (item.itemId) {
                ...
                R.id.add_diary -> {
                    // Retrofit 서비스 호출
                    apiobject.api.diarywriteEx(3,getCurrentFormattedDate()).enqueue(object :
...
//        radarDataSet.lineWidth = 2f // 기본값은 1f
        ...
        yAxis.setDrawLabels(false) // Y축 레이블 숨기기
...
//        }
        ...
        val barDataSet = BarDataSet(barEntries, "카테고리별 소비 금액").apply {
            color = Color.parseColor("#e49995")
            valueTextColor = Color.BLACK
            setDrawValues(false) // 라벨(값) 비활성화
        }
